...
In terms of security properties, the design of Composite ML-DSA is SUF-CMA secure only if both components are SUF-CMA secure -- specifically, that means the ML-DSA and EdDSA combinations are SUF-CMA, but the ML-DSA and RSA or ECDSA combinations are only EUF-CMA. This means that replacing an ML-DSA signature with a Composite ML-DSA signature with RSA or ECDSA could be considered a reduction in security if your application is sensitive to the difference between SUF and EUF security. Applications requiring SUF-CMA security are RECOMMENDED to use the ML-DSA + EdDSA combinations.
